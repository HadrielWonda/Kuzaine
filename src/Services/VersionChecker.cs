...
public static class VersionChecker
{
    public static void CheckForLatestVersion()
    {
        try
        {
            ...
            //if (!isPrelease)
            //{
            var client = new RestClient("https://github.com/hadrielwonda/kuzaine/releases/latest");
            ...
            if (installedVersion != latestVersion)
                AnsiConsole.MarkupLine(@$"{Environment.NewLine}[bold seagreen2]Kuzaine version '{installedVersion}' is older than that of the runtime '{latestVersion}'. Update the tools for the latest features and bug fixes (`dotnet tool update -g kuzaine`).[/]{Environment.NewLine}");
            //}
        }
        catch (Exception)
        {
